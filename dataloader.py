...
class DataGenerator(Dataset):
    def __init__(self, cover_dir, stego_dir):
        ...
        self.filename_list_2 = stego_list
    ...
    def __getitem__(self, index):
        batch = np.empty(shape=(2, self.img_shape[0], self.img_shape[1], 3), dtype='uint8')

        batch[0, :, :, :] = imageio.imread(join(self.cover_path, self.filename_list[index]))
        batch[1, :, :, :] = imageio.imread(join(self.stego_path, self.filename_list_2[index]))
